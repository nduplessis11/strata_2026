...
namespace strata::err::detail {
    ...
    /**
     * @brief Concept for types convertible to std::string_view, excluding raw arrays.
     *
     * This concept matches types like `const char*`, `std::string_view`, and `std::string`,
     * but rejects array types (e.g., `char[N]`) so that string literals bind to the
     * format-string overloads instead.
     *
     * @tparam T The type to check.
     */
    template<class T>
    concept string_viewable =
        std::constructible_from<std::string_view, T> &&
        (!std::is_array_v<std::remove_reference_t<T>>);

    /**
     * @brief Wrapper that couples a compile-time checked format string with a call-site source location.
     *
     ...
     *
     * The constructor is `consteval`, so the format string must be a compile-time string
    ...
    struct fmt_with_loc_impl {
        ...
        /**
         * @brief Construct from a string literal and capture the call-site location by default.
         *
         * @param s String literal used as the format string.
         * @param l Call-site location (defaults to `std::source_location::current()`).
         */
        template<size_t N>
     ...
     *
     * This alias normalizes the `Args...` list through `std::type_identity_t` to avoid
     * surprising deductions caused by references/cv-qualifiers.
     *
     * @tparam Args Format argument types.
     */
    template<class... Args>
    using fmt_with_loc =
        fmt_with_loc_impl<std::type_identity_t<Args>...>;
}
...
namespace strata::err {
     ...
     *
     * The stack trace is captured with `std::stacktrace::current(skip)`, which skips
     * the top @p skip frames to keep the output focused on the caller path.
     *
     * Any exception thrown while streaming the stack trace is swallowed.
     *
     * @param skip Number of stack frames to skip (default: 2, skips fail_expect and its caller).
     *
     ...
     *
     * If @p predicate is false, prints the call-site location (file:line:column function)
     * followed by @p msg (as plain text) to `stderr`, then terminates via fail_expect().
     *
     * This overload accepts any message type constructible as `std::string_view`
     * (for example `const char*`, `std::string_view`, or `std::string`) while rejecting
     * array types (so string literals do not bind as arrays).
     *
     ...
     * @param predicate Condition that must be true.
     * @param f         Wrapper containing a compile-time checked format string and call-site location.
        ...
        Args&&... args) noexcept
    {
        ...
        try {
            std::print(stderr, "{}:{}:{} {} -- ",
                f.loc.file_name(), f.loc.line(), f.loc.column(), f.loc.function_name());
     ...
     * @ingroup err_assert
     *
     * @brief Ensures that a predicate is true, otherwise throws (plain-text message overload).
     *
     * If @p predicate is false, throws `std::runtime_error` with a message that includes
     * the call-site location (file:line:column function) followed by @p msg.
     *
     * This overload accepts any message type constructible as `std::string_view`
     * (for example `const char*`, `std::string_view`, or `std::string`) while
     * rejecting array types.
     *
     * Unlike expect(), ensure() reports failure by throwing rather than terminating.
     *
     * @tparam Msg      Message type satisfying `detail::string_viewable`.
     * @param predicate Condition that must be true.
     ...
     * @ingroup err_assert
     *
     * @brief Ensures that a predicate is true, otherwise throws (formatted message overload).
     *
     * If @p predicate is false, formats a message with `std::format(f.fmt, args...)`
     * and throws `std::runtime_error` containing the call-site location followed by the
     * formatted message.
     *
     * The format string is compile-time checked by `detail::fmt_with_loc<Args...>`,
     * and the call-site location is captured automatically via the wrapper's
        ...
        Args&&... args)
    {
        ...
        throw std::runtime_error(
            std::format("{}:{}:{} {} -- {}",
                f.loc.file_name(), f.loc.line(), f.loc.column(),
                f.loc.function_name(),
     ...
     *
     * In debug builds (when `NDEBUG` is not defined), evaluates @p predicate by invoking it.
     * If it returns `false`, evaluates @p msg (only on failure) to produce a message that is
     * convertible to `std::string_view` (see `detail::string_viewable`), then reports the
     * failure via the plain-text `expect(false, msg(), loc)` overload (terminating).
     *
     * This allows @p msg to return `const char*`, `std::string_view`, `std::string`, etc.,
     * without requiring an owning `std::string`.
     *
     * In release builds (when `NDEBUG` is defined), this overload is a no-op and neither
     * callable is invoked.
     *
     * @tparam Predicate Callable type returning `bool`.
     * @tparam Message   Callable type returning a `detail::string_viewable` message.
    ...
    template<std::invocable Predicate, std::invocable Message>
        requires
    std::same_as<std::invoke_result_t<Predicate>, bool>&&
        detail::string_viewable<std::invoke_result_t<Message>>
        inline void debug_expect(
            Predicate&& predicate,
            Message&& msg,
            std::source_location loc = std::source_location::current())
    {
        if (std::forward<Predicate>(predicate)()) [[likely]] return;
     ...
     *
     * In debug builds (when `NDEBUG` is not defined), evaluates @p predicate by invoking it.
     * If it returns `false`, reports failure via the plain-text `expect(false, msg, loc)`
     * overload (terminating).
     *
     * In release builds (when `NDEBUG` is defined), this overload is a no-op and the
     * predicate/message are not evaluated.
     *
     * @tparam Predicate Callable type returning `bool`.
     * @tparam Msg       Message type satisfying `detail::string_viewable`.
     * @param predicate  Callable evaluated to check the condition.
     ...
     * @ingroup err_assert
     *
     * @brief Debug-only assertion with lazy predicate and an eager formatted message (no args).
     *
     * In debug builds (when `NDEBUG` is not defined), evaluates @p predicate by invoking it.
     * If it returns `false`, reports failure via the formatted `expect(false, fmt)` overload
     * and terminates.
     *
     * This overload exists to support calls like:
     * `debug_expect([&]{ return ok(); }, "failed");`
     *
     * where the format string has **no format arguments**. The call-site source location is
     * captured automatically by `detail::fmt_with_loc<>`.
     *
     * In release builds, this overload is replaced with a no-op implementation and neither
     * the predicate nor the format wrapper are evaluated.
     *
     * @tparam Predicate Callable type returning `bool`.
     * @param predicate  Callable evaluated to check the condition.
     * @param fmt        Wrapper containing a compile-time checked format string (no arguments)
     *                   and the call-site source location.
     *
     * @note Terminates the program on failure (debug builds only).
     */
     ...
     * @ingroup err_assert
     *
     * @brief Debug-only assertion with lazy predicate and lazy message (release no-op).
     *
     * This is the release-build implementation (when `NDEBUG` is defined). It performs no checks
     * and does not evaluate either callable.
     *
     * The `Message` callable is permitted to return any type satisfying `detail::string_viewable`,
     * but it is not invoked in release builds.
     *
     * @tparam Predicate Callable type returning `bool`.
     * @tparam Message   Callable type returning a `detail::string_viewable` message.
     ...
     * @ingroup err_assert
     *
     * @brief Debug-only assertion with lazy predicate and eager plain-text message (release no-op).
     *
     * This is the release-build implementation (when `NDEBUG` is defined). It does nothing,
     * and the provided predicate/message are not evaluated.
     *
     * @tparam Predicate Callable type returning `bool`.
     * @tparam Msg       Message type satisfying `detail::string_viewable`.
     * @param predicate  Unused.
     ...
     * @ingroup err_assert
     *
     * @brief Debug-only assertion with lazy predicate and an eager formatted message (release no-op).
     *
     * This is the release-build implementation (when `NDEBUG` is defined). It performs no checks
     * and does not evaluate the predicate or the format wrapper.
     *
     * @tparam Predicate Callable type returning `bool`.
     * @param predicate  Unused.
     * @param fmt        Unused.
